...
export default function FavoritesListScreen({route, navigation}) {
    ...
    const { favoriteFileTitle, setFavoriteFileNames, setSelectedBook } = bookCtx;
    ...
    if (title === null) {
        return (
            <View style={styles.fallbackContainer}>
                <Text style={styles.fallbackText}>No favorite added yet!</Text>
            </View>
        );
    }
                            ...
                            <Pressable onPress={() => readBookHandler(item)} android_ripple={{color: KolorKit.blackBlueTheme.btnNavigationHover}} style={({pressed}) => [styles.bookPressableContainer, pressed && styles.pressed]}>
                                <View style={styles.bookContainer}>
                                    ...
                                    <View style={styles.bookOptionsContainer}>
                                        <Fontisto style={{padding: 6}} onPress={() => removeFromFavoritesHandler(item)} name="favorite" size={26} color={KolorKit.blackBlueTheme.yellow400} />
